...
dbxMiddleware = (options) ->
  ...
  (request, response, next) ->
    ...
    client.authenticate (error) ->
      ...
      request.dropboxClient = client
      next()

# Extracts and resets the OAuth state for this user.
dbxMiddleware.pullOauthState = (request, response) ->
  # cookieSession middleware.
  if request.session and request.session.dropbox_js_oauth_state
    oauthState = request.session.dropbox_js_oauth_state
    delete request.session.dropbox_js_oauth_state
    return oauthState

  # cookieParser middleware / express.js cookie API.
...
dbxMiddleware.setOauthState = (request, response, state) ->
  # cookieSession middleware.
  if response.session
    response.session.dropbox_js_oauth_state = state
    return

  # Fall back to the express.js cookie API.
